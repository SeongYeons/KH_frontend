$(document).ready(() => {
    ...
    // $('div h3').html('<i>h3 값 변경</i>');
    // $('div h3').text('<i>h3 값 변경</i>');
    $('div .child').css('background-color', 'yellow');

    // 5. 속성 선택자
    $('#div1 input[class]').css('background-color', 'pink');
    $('#div1 input[type=text]').val('임의의 값 설정');
    $('#div1 input[class~=test1]').val('1234567');
    $('#div1 input[type^=ra]').prop('checked', true);
    $('#div1 input[type$=box]').prop('checked', true);
    $('#div1 input[class*=st3]').css('width', '100px').css('height', '100px').val('10000');
    ...
    // mouseenter() : 마우스가 요소 내부로 들어왔을 때 발생하는 
    ...
    // mouseout() : 마우스가 요소 외부로 나갔을 때 발생하는
    //              이벤트를 처리할 핸들러를 등록하는 메소드이다.
    ...
    // change() : checkbox, radio 요소들의 상태가 변경될 때 발생하는
    //            이벤트를 처리할 핸들러를 등록하는 메소드이다.
